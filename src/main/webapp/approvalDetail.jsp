...
<body>
    <h1>申请免修不免考</h1>
    <center>
        <form action="eventCommitServlet" method="post" accept-charset=“UTF-8”>
            <label>事件号：</label>
            ...
            <!-- 假设服务器上已经有名为example.pdf的文件 -->
            <a href="downLoadFileServlet?fileName=${fileName}" onclick="downloadFile(${fileName})">下载文件</a>
            <label>审批意见：</label>
...
<script>
    function downloadFile(filename) {
        var url = getDownloadURL(filename); // 获取下载URL
        window.location.href = url; // 使用获取到的URL进行文件下载
    }

    function getDownloadURL(filename) {
        // 获取文件下载的servlet地址
        var servletUrl = "downloadFileServlet";
        // 将文件名作为参数拼接在URL中
        var url = servletUrl + "?fileName=" + filename;
        alert(url)
        return url;
    }
</script>
