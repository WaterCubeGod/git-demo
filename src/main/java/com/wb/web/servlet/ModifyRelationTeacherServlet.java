package com.wb.web.servlet;

import com.wb.pojo.Teacher;
import com.wb.service.TeacherService;
import com.wb.service.TeachesService;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

@WebServlet("/modifyRelationteacher")
public class ModifyRelationTeacherServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        response.setContentType("text/html;charset=utf-8");
        request.setCharacterEncoding("utf-8");

        String id = request.getParameter("id");

        Teacher teacher = TeacherService.selectById(id);

        List<String> courseIds = TeachesService.selectNone();

        List<String> courseNames = new ArrayList<>();

        for (String courseId : courseIds) {
            String courseName = TeachesService.selectName(courseId);
            courseNames.add(courseName);
        }

        request.setAttribute("id",id);
        request.setAttribute("name",teacher.getName());
        request.setAttribute("courseNames",courseNames);
        //转发到manageFlow.jsp
        request.getRequestDispatcher("/modifyTeacher.jsp").forward(request,response);


    }

    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {

        this.doGet(request, response);
    }
}
